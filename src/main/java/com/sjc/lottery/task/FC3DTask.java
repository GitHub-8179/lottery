...
//    @Autowired
    ...
    private String urlPath3d = "http://www.cwl.gov.cn/cwl_admin/kjxx/findDrawNotice?name=3d&issueCount="+num;

    @Scheduled(cron = "0 59 21 * * ?")
    public Map insertDoubleBall() {
        String json =   requestDoubleBall(urlPath3d);
        if(json.length()>0) {
    ...
    public String requestDoubleBall(String urlPath) {
        ...
        try {
            ...
            httpConn.setRequestProperty("GET", "/cwl_admin/kjxx/findDrawNotice?name=3d&issueCount="+num+" HTTP/1.1");//文本信息
