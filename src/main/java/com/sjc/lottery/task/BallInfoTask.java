...
/**
 * Description: lottery
 * Created by s on 2019/5/30 10:31
 */
@Configuration//配置类
@EnableScheduling//支持定时任务类
public class BallInfoTask {
    ...
    private String http = "http://www.cwl.gov.cn/";
    /**
     * 双色球
     *  * 每周二、四、日 21:15开奖
     */
    ...
    public void ssq(){
        String url = "cwl_admin/kjxx/findDrawNotice?name=ssq&issueCount="+num;
        insertLottery(url,"ssq","1");
    }
    /**
     *  * 七乐彩
     *  * 每周一、三、五 21:15开奖
     */
    @Scheduled(cron = "0 35 21 * * 1,3,5 ")//每隔1秒执行一次
    public void qlc(){
        String url = "cwl_admin/kjxx/findDrawNotice?name=qlc&issueCount="+num;
        insertLottery(url,"qlc","2");
    }
    /**
     * * 福彩3d
     *  * [ 每日 21:15开奖
    ...
    public void fc3d(){
        String url = "cwl_admin/kjxx/findDrawNotice?name=3d&issueCount="+num;
        insertLottery(url,"fc3d","3");
    }
    ...
    public void ssqDetail(){
        String url = "cwl_admin/kjxx/findDrawNotice?name=ssq&issueCount="+num;
        insertLotteryDetail(url,"ssq","1");
    }
    /**
     *  * 七乐彩
     *  * 每周一、三、五 21:15开奖
     */
    @Scheduled(cron = "0 15 22 * * 1,3,5 ")//每隔1秒执行一次
    public void qlcDetail(){
        String url = "cwl_admin/kjxx/findDrawNotice?name=qlc&issueCount="+num;
        insertLotteryDetail(url,"qlc","2");
    }
    /**
     * * 福彩3d
     *  * [ 每日 21:15开奖
    ...
    public void fc3dDetail(){
        String url = "cwl_admin/kjxx/findDrawNotice?name=3d&issueCount="+num;
        insertLotteryDetail(url,"fc3d","3");
    }

    public void insertLottery(String url,String referer,String lotteryType){
        String json = requestBall(url,referer);
        if(json.length()>0) {
    ...
    public void insertLotteryDetail(String url,String referer,String lotteryType){
        String json = requestBall(url,"ssq");
        if(json.length()>0) {
            JSONObject jsonobj = JSON.parseObject(json);
            JSONArray resultJson = jsonobj.getJSONArray("result");
            ...
            for (int i = 0, num = resultJson.size(); i < num; i++) {
                ...
                criteria.andLotteryPeriodEqualTo(lotteryPeriod);
                criteria.andLotteryTypeEqualTo(lotteryType);

                lotteryMapper.updateByExample(lottery,example);

                List<Lottery> lotteryList =  lotteryMapper.selBallInfo(lottery);
                ...
                for (Lottery lottery1 : lotteryList) {
                    ...
                    for (int k = 0, numk = prizegradesJSONArray.size(); k < numk; k++) {
                        ...
                        lotteryDetail.setLotteryId(lottery1.getLotteryId());
    ...
    public String requestBall(String urlPath,String referer) {
        ...
        try {
            ...
            httpConn.setRequestProperty("GET", urlPath+" HTTP/1.1");//文本信息
            ...
            httpConn.setRequestProperty("Referer", "http://www.cwl.gov.cn/kjxx/"+referer+"/kjgg/");//文本信息
