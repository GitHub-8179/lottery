...
/**
 * Description: lottery
 * Created by s on 2019/5/26 17:59
 * 七乐彩
 * 每周一、三、五 21:15开奖
 */
@Service
...
//    @Autowired
    ...
    private String qlcUrlPath = "http://www.cwl.gov.cn/cwl_admin/kjxx/findDrawNotice?name=qlc&issueCount="+num;

    @Scheduled(cron = "0 59 21 * * 1,3,5 ")//每隔1秒执行一次
    public Map insertDoubleBall() {
        String json =   requestDoubleBall(qlcUrlPath);
        if(json.length()>0) {
    ...
    public String requestDoubleBall(String urlPath) {
        ...
        try {
            ...
            httpConn.setRequestProperty("GET", "/cwl_admin/qlc/findDrawNotice?name=qlc&issueCount="+num+" HTTP/1.1");//文本信息
            ...
            httpConn.setRequestProperty("Referer", "http://www.cwl.gov.cn/cwl_admin/qlc/");//文本信息
